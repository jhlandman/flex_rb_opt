...
# Getting sample covariance matrix -----------------------------------------------------------------
...
# for the top 40: <CAN USE A PURRR::MAP() OF THIS LOGIC TO FILTER>
dat_check <- model_rebal_dat$data[[1]]
last_date <- dat_check[dat_check$date == max(dat_check$date),]
forty_ids <- last_date[last_date$histadj_top40_weight > 0,]$name
filtered <- dat_check[dat_check$name %in% forty_ids,]
get_cov(filtered)

# would be this for full... But only want for top40
covars = purrr::map(model_rebal_dat$data, get_cov)

# Minimum variance portfolio optimisation ----------------------------------------------------------
# program is quadratic so using quadprog

get_weights_now <- function(Dmat_in){
  ...
  df <- tibble(names = colnames(Dmat), weights = qp$solution)
  df$weights[df$weights < 0] <- 0
  return(df)
}
list_weight = purrr::map(covars, get_weights_now)
names(list_weight) <- model_rebal_dat$month_index

out = dplyr::bind_rows(list_weight, .id = "month_index")

ggplot2::ggplot(out, ggplot2::aes(x=weights)) + 
  ggplot2::geom_density()

# can join to full ret before the fact to find the 40 and 100 largest stocks, 
# then look at the 

# Equal risk contribution portfolio optimisation ---------------------------------------------------
